...
public class Regression_Scenario_CAP_21
        ...
        implements CMPLogin_Page.ILogin, CMPHome_Page.IHome, CommonInfo_Page.ICommonInfo, Names_Page.INames, Rating_Page.IRating, Issue_Page.IIssue_Page, IssuingInfo_Page.IIssuingInfo, PolicyForms_Page.IPolicyForms, FEIN_Page.IFEIN
{
    ...
    public void Regression_Scenario_CAP_21_Transaction_001_NBQUNBIS() throws IOException, InvalidFormatException, ParserConfigurationException, SAXException, InterruptedException {
        initialTransactionName = "NBQUNBIS";
        excelFileName = "Regression_Scenario_CAP_21_Test_Datafile";
        ...
        if (testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("$$NA$$") || testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("Fail")) {
            ...
            whileViewing().theCapitalAssetPage().theUser_().addOptionalCovgs(testDataAsMap,"Regression_Scenario_CAP_21");
    ...
    @Test(enabled = true,  priority = 1, dependsOnMethods = "Regression_Scenario_CAP_21_Transaction_001_NBQUNBIS" )
    public void Regression_Scenario_CAP_21_Transaction_002_PCNM1() throws IOException, InvalidFormatException, ParserConfigurationException, SAXException, InterruptedException
    {
        excelFileName = "Regression_Scenario_CAP_21_Test_Datafile";
        excelFilePath = utils.configReader().getExcelFilePath(excelFileName);
        testDataAsMap = utils.excelFileUtil().getExcelDataAsMapWithMultipleValues(excelFilePath, "PCNM1");
        ...
        if(testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("$$NA$$") || testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("Fail"))
        {
            ...
            whileViewing().theUnderwriterPage().theUser_().enterUnderwriterDetails(testDataAsMap);
            whileViewing().theCapitalAssetPage().theUser_().enterCapitalAssetPolicyDetail(testDataAsMap);
            whileViewing().theCAPLocationPage().theUser_().PCNMLocationCovgs(testDataAsMap);
    ...
    @Test(enabled = true,  priority = 2, dependsOnMethods = "Regression_Scenario_CAP_21_Transaction_002_PCNM1" )
    public void Regression_Scenario_CAP_22_Transaction_002_PCNM2() throws IOException, InvalidFormatException, ParserConfigurationException, SAXException, InterruptedException
    {
        excelFileName = "Regression_Scenario_CAP_21_Test_Datafile";
        excelFilePath = utils.configReader().getExcelFilePath(excelFileName);
        testDataAsMap = utils.excelFileUtil().getExcelDataAsMapWithMultipleValues(excelFilePath, "PCNM2");
