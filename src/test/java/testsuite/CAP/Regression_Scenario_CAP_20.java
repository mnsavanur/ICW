...
public class Regression_Scenario_CAP_20
    ...
    public void Regression_Scenario_CAP_20_Transaction_01_NBQUNBIS() throws IOException, InvalidFormatException, ParserConfigurationException, SAXException, InterruptedException {
        initialTransactionName = "NBQUNBIS";
        excelFileName = "Regression_Scenario_CAP_20_Test_Datafile";
        ...
        if (testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("$$NA$$") || testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("Fail")) {
            ...
            //following methods are called for CAP
            ...
            whileViewing().theCAPLocationPage().theUser_().addCoverage(testDataAsMap);
    ...
    @Test(enabled = true, priority = 1, dependsOnMethods = "Regression_Scenario_CAP_20_Transaction_01_NBQUNBIS")
    public void Regression_Scenario_CAP_20_Transaction_02_CANC() throws IOException, InvalidFormatException, ParserConfigurationException, SAXException, InterruptedException {
        excelFileName = "Regression_Scenario_CAP_20_Test_Datafile";
        excelFilePath = utils.configReader().getExcelFilePath(excelFileName);
    ...
    @Test(enabled = true, priority = 2, dependsOnMethods = "Regression_Scenario_CAP_20_Transaction_02_CANC")
    public void Regression_Scenario_CAP_20_Transaction_03_REISSUE() throws IOException, InvalidFormatException, ParserConfigurationException, SAXException, InterruptedException {
        excelFileName = "Regression_Scenario_CAP_20_Test_Datafile";
        excelFilePath = utils.configReader().getExcelFilePath(excelFileName);
        testDataAsMap = utils.excelFileUtil().getExcelDataAsMapWithMultipleValues(excelFilePath, "REISSUE");
        ...
        if (testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("$$NA$$") || testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("Fail")) {
            ...
            whileViewing().theCommonInfoPage().theUser_().changeCommonPolicyDetailsREISSUE(testDataAsMap);
            whileViewing().theTransactionInformationPage().theUser_().enterPolicyTypeMonoline(testDataAsMap);
            whileViewing().theRatingPage().theUser_().ratePolicyWithoutVerification(testDataAsMap);
            whileViewing().theIssuingInfo().theUser_().fillIssuingInfoDetails(testDataAsMap, sanNumber, "NBQUNBIS");
            whileViewing().theBillingPage().theUser_().fillBillingPageDetails(testDataAsMap);
            whileViewing().theMiscInformationPage().theUser_().fillMiscInformation(testDataAsMap, sanNumber);
    ...
    public void afterMethod(ITestResult testResult) throws ParserConfigurationException, InvalidFormatException, SAXException, IOException {
        ...
        scenarioName = scenarioName.replace("Smoke_", "");
