...
public class Regression_Scenario_CP_11
    ...
    public void Regression_Scenario_CP_11_Transaction_01_NBQUNBIS() throws Exception {
        initialTransactionName = "NBQUNBIS";
        excelFileName = "Regression_Scenario_CP_11";
    ...
    @Test(enabled = true, priority = 1, dependsOnMethods = "Regression_Scenario_CP_11_Transaction_01_NBQUNBIS")
    public void Regression_Scenario_CP_11_Transaction_02_CANC() throws Exception {
        excelFileName = "Regression_Scenario_CP_11";
        excelFilePath = utils.configReader().getExcelFilePath(excelFileName);
    ...
    @Test(enabled = true, priority = 2, dependsOnMethods = "Regression_Scenario_CP_11_Transaction_02_CANC")
    public void Regression_Scenario_CP_11_Transaction_03_VNEW() throws Exception {
        excelFileName = "Regression_Scenario_CP_11";
        excelFilePath = utils.configReader().getExcelFilePath(excelFileName);
        testDataAsMap = utils.excelFileUtil().getExcelDataAsMapWithMultipleValues(excelFilePath, "VNEW");
        ...
        if (testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("$$NA$$") || testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("Fail")) {
           // whileViewing().theCMPLoginPage().theUser_().StartRecording(testDataAsMap.get("TestCase_ID").get(0));
            whileViewing().theCMPLoginPage().theUser_().logsIntoPolicyDecisions(utils.configReader().getURL(), testDataAsMap.get("userNameTxt").get(0), testDataAsMap.get("passwordTxt").get(0));
            whileViewing().theCMPHomePage().theUser_().performActionsOnPolicy(testDataAsMap,"VNEW",sanNumber);
