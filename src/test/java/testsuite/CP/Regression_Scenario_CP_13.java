...
public class Regression_Scenario_CP_13
    ...
    public void Regression_Scenario_CP_13_Transaction_01_NBQUNBIS() throws Exception {
        initialTransactionName = "NBQUNBIS";
        excelFileName = "Regression_Scenario_CP_13";
    ...
    @Test(enabled = true, priority = 1 , dependsOnMethods = "Regression_Scenario_CP_13_Transaction_01_NBQUNBIS")
    public void Regression_Scenario_CP_13_Transaction_02_CRTC() throws Exception {
        excelFileName = "Regression_Scenario_CP_13";
        excelFilePath = utils.configReader().getExcelFilePath(excelFileName);
        testDataAsMap = utils.excelFileUtil().getExcelDataAsMapWithMultipleValues(excelFilePath, "CRTC");
        ...
        if(testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("$$NA$$") || testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("Fail"))
        {
            ...
            whileViewing().theCMPHomePage().theUser_().PerformCertifiedCopyofPolicy(testDataAsMap, sanNumber);
        ...
        if (transactionStatus) {
            ...
            if (generateBeyondCompareScriptAndReport) {
                ...
                if (previousSPPolicyOrSANNumber.equalsIgnoreCase("")) {
                    ...
                    previousSPPolicyOrSANNumber = utils.excelFileUtil().getsSPPolicyOrQuoteNumber(excelFilePath + excelFileName, "CRTC", previousSPNumber, transactionNameAlsoTheTestDataSheetName, scenarioName);
                }
