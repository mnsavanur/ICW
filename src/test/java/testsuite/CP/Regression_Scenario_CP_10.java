...
public class Regression_Scenario_CP_10
    ...
    public void Regression_Scenario_CP_10_Transaction_01_NBQU() throws Exception {
        initialTransactionName = "NBQU";
        excelFileName = "Regression_Scenario_CP_10";
        ...
        if (testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("$$NA$$") || testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("Fail")) {
            ...
            whileViewing().theCMPHomePage().theUser_().checkNBQUTransactionStatus(sanNumber);
            whileViewing().theCMPLoginPage().theUser_().logOut();
        }
    }

    @Test(enabled = true,  priority = 1, dependsOnMethods = "Regression_Scenario_CP_10_Transaction_01_NBQU" )
    public void Regression_Scenario_CP_10_Transaction_02_DECLINE() throws Exception {
        excelFileName = "Regression_Scenario_CP_10";
        excelFilePath = utils.configReader().getExcelFilePath(excelFileName);
        testDataAsMap = utils.excelFileUtil().getExcelDataAsMapWithMultipleValues(excelFilePath, "DECLINE");
        ...
        if(testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("$$NA$$") || testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("Fail"))
        {
            whileViewing().theCMPLoginPage().theUser_().StartRecording(testDataAsMap.get("TestCase_ID").get(0));
            whileViewing().theCMPLoginPage().theUser_().logsIntoPolicyDecisions(utils.configReader().getURL(), testDataAsMap.get("userNameTxt").get(0), testDataAsMap.get("passwordTxt").get(0));
            whileViewing().theCMPHomePage().theUser_().performTransaction(testDataAsMap, "DECLINE", sanNumber);
            whileViewing().thePerformTransactionPage().theUser_().enterDeclineInfo(testDataAsMap);
