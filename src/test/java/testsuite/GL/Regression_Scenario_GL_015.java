        ...
        implements CMPLogin_Page.ILogin, CMPHome_Page.IHome, CommonInfo_Page.ICommonInfo, Names_Page.INames, Rating_Page.IRating, Issue_Page.IIssue_Page, IssuingInfo_Page.IIssuingInfo, PolicyForms_Page.IPolicyForms, FEIN_Page.IFEIN
{
    ...
    public void Regression_Scenario_Transaction_015_NBIS() throws IOException, InvalidFormatException, ParserConfigurationException, SAXException, InterruptedException
    {
        ...
        if(testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("$$NA$$") || testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("Fail"))
        {
            ...
            //  whileViewing().theIssuingInfo().theUser_().convertToIssueNBQU();
          //  whileViewing().theRatingPage().theUser_().ratePolicyWithoutVerificationNBQU(testDataAsMap);
          //  whileViewing().thePolicyFormsPage().theUser_().fillsInPolicyForms2(testDataAsMap);
    ...
    /*@Test(enabled = true,  priority = 1, dependsOnMethods = "Smoke_Scenario_Transaction_002_NBIS" )
    public void Smoke_Scenario_Transaction_002_PCNM() throws IOException, InvalidFormatException, ParserConfigurationException, SAXException, InterruptedException
    {
        excelFileName = "GL_Smoke_02";
        excelFilePath = utils.configReader().getExcelFilePath(excelFileName);
        testDataAsMap = utils.excelFileUtil().getExcelDataAsMapWithMultipleValues(excelFilePath, "PCNM");
...
/*
    @Test(enabled = true, priority = 1 , dependsOnMethods = "Smoke_Scenario_Transaction_002_NBIS")
    public void Smoke_Scenario_Transaction_002_CANC() throws IOException, InvalidFormatException, ParserConfigurationException, SAXException, InterruptedException
    {
        excelFileName = "GL_Smoke_02";
        excelFilePath = utils.configReader().getExcelFilePath(excelFileName);
    ...
    @Test(enabled = true, priority = 2 , dependsOnMethods = "Smoke_Scenario_Transaction_002_CANC")
    public void Smoke_Scenario_Transaction_002_CARS() throws IOException, InvalidFormatException, ParserConfigurationException, SAXException, InterruptedException
    {
        excelFileName = "GL_Smoke_02";
        excelFilePath = utils.configReader().getExcelFilePath(excelFileName);
